...
try:
    ...
    fluid_synth_get_chorus_speed = cfunc(
        "fluid_synth_get_chorus_speed", c_double, ("synth", c_void_p, 1)
    )

    fluid_synth_get_chorus_depth = cfunc(
    ...
    fluid_synth_get_chorus_speed_Hz = cfunc(
        "fluid_synth_get_chorus_speed_Hz", c_double, ("synth", c_void_p, 1)
    )

    fluid_synth_get_chorus_depth_ms = cfunc(
...
class Synth:
    ...
    def start(self, driver=None, device=None, midi_driver=None):
        ...
        if driver is not None:
            ...
            if device is not None:
                fluid_settings_setstr(
                    self.settings,
                    str("audio.%s.device" % (driver)).encode(),
                    device.encode(),
                )
            self.audio_driver = new_fluid_audio_driver(self.settings, self.synth)
        if midi_driver is not None:
            assert midi_driver in [
                "alsa_seq",
                "alsa_raw",
                "oss",
                "winmidi",
    ...
    def router_end(self):
        if self.router is not None:
            ...
            if (
                fluid_midi_router_add_rule(
                    self.router, self.router.cmd_rule, self.router.cmd_rule_type
                )
                < 0
            ):
                delete_fluid_midi_router_rule(self.router.cmd_rule)
    ...
    def set_reverb(self, roomsize=-1.0, damping=-1.0, width=-1.0, level=-1.0):
        ...
        try:
            ...
            return fluid_synth_set_reverb_full(
                self.synth, set, roomsize, damping, width, level
            )
...
class Sequencer:
    ...
    def register_client(self, name, callback, data=None):
        ...
        response = fluid_sequencer_register_client(
            self.sequencer, name.encode(), c_callback, data
        )
        if response == FLUID_FAILED:
    ...
    def note(
        self, time, channel, key, velocity, duration, source=-1, dest=-1, absolute=True
    ):
