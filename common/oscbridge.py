...
def start_bridge(remote_ip):
    ...
    from_synapse = OSCForwarder(
        "localhost", synapse_send_port, remote_ip, synapse_send_port
    )
    to_synapse = OSCForwarder(
        my_ip, bridge_listen_port, "localhost", synapse_listen_port
    )
