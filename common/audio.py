...
class Audio(object):
    ...
    def on_update(self):
        ...
        if self.input_stream:
            try:
                num_frames = (
                    self.input_stream.get_read_available()
                )  # number of frames to ask for
                if num_frames:
        ...
        if self.generator and num_frames != 0:
            (data, continue_flag) = self.generator.generate(
                num_frames, self.num_channels
            )

            # make sure we got the correct number of frames that we requested
            assert len(data) == num_frames * self.num_channels, (
                "asked for (%d * %d) frames but got %d"
                % (num_frames, self.num_channels, len(data))
            )
...
"""
    ...
    def add_device(arr, io_type, dev):
        ...
        info["name"] = dev["name"]
        info["latency"] = (
            dev["defaultLow" + io_type + "Latency"],
...
def print_audio_devices():
    ...
    for d in devs["output"]:
        print(
            "{index:>5}: {name:<40} {channels:<6} {latency[0]:.3f} - {latency[1]:.3f}".format(
                **d
            )
        )
